...
opts.Add(EnumVariable('cpp_version', 'C++ Version', 'c++14', [ 'c++14', 'c++17' ]))
...
if env['platform'] == 'linux':
    ...
    env.Append(CXXFLAGS=['-std=' + cpp_version])
    if env['target'] == 'debug':
...
output_name_suffix = '.{}.{}.{}'.format(
                        env['platform'],
                        env['target'],
                        arch_suffix)

cpp_bindings_libname = f'libgodot-cpp{output_name_suffix}'
...
lib_name = f"{env['target_path']}{env['target_name']}{output_name_suffix}"

# determine to link as shared or static library
if env['platform'] == "ios":
    library = env.StaticLibrary(target=lib_name, source=sources)
else:
